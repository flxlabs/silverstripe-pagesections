(function ($) {
	$.entwine('ss', function ($) {
		...
		$('.view-detail-dialog .view-detail-form').entwine({
			onsubmit: function () {
				var dialog = this.closest('.view-detail-dialog').children('.ui-dialog-content');
		...
		$('.treeview-pagesections').entwine({
			...
			onadd: function () {
				...
				$treeView.find('.treeview-item').each(function () {
					...
					// Edit button
					$item
						.find('> .treeview-item-actions .edit-button')
						.click(function () {
							var dialog = $('<div></div>')
								.appendTo('body')
								.dialog({
							...
							dialog
								...
								.loadDialog($.get($treeView.data('url') + '/detail?ID=' + itemId));
							dialog.data('treeview', $treeView);
						});
					...
					// Dropping targets
					$item.find('> .treeview-item-flow .treeview-item-reorder div').each(function () {
						$(this).droppable({
							hoverClass: 'state-active',
							tolerance: 'pointer',
